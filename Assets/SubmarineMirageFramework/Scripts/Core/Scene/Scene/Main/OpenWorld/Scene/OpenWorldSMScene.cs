...
namespace SubmarineMirage.Scene {
	...
	public abstract class OpenWorldSMScene : MainSMScene {
		...
		protected abstract int _loadLowChunkRadius	{ get; set; }
		protected abstract int _loadHighChunkRadius	{ get; set; }
		...
		public OpenWorldCenter _center	{ get; private set; }
		...
		public OpenWorldSMScene() {
			...
			_updateEvent.AddLast().Subscribe( _ => Update() );
		}
		...
		bool UpdateLow( Vector3Int center, out bool isInside ) {
			var start = center.Sub( _loadLowChunkRadius );
			var end = center.Add( _loadLowChunkRadius );
		...
		bool UpdateHigh( Vector3Int center, Vector3 centerScale, bool isLowInside ) {
			if ( isLowInside ) {
				var radius = _loadHighChunkRadius - 1;
				var start = center;
				var end = start;

				if ( radius > 0 ) {
					end += centerScale.OutsideDirection() * radius;
...
// TODO : 重複呼びを防止するキューに変更
			...
			if ( _lastLowStartPosition.HasValue ) {
				SetNextLoadType(
					SMChunkLoadType.Low, _lastLowStartPosition.Value, _lastLowEndPosition.Value );
			}
			if ( _lastHighStartPosition.HasValue ) {
				SetNextLoadType(
					SMChunkLoadType.High, _lastHighStartPosition.Value, _lastHighEndPosition.Value );
			}
			...
			await _chunkData
				...
				.Select( d => {
					if ( d._nextLoadType == null ) {
						d._nextLoadType = SMChunkLoadType.Unload;
//						SMLog.Debug( d._name );
					}
					return d.Reload();
				} );
			...
			_isReloading = false;
			if ( _isRequestReload )	{ Reload().Forget(); }
		}


		void SetNextLoadType( SMChunkLoadType type, Vector3Int start, Vector3Int end ) {
			for ( var x = start.x; x <= end.x; x++ ) {
				for ( var y = start.y; y <= end.y; y++ ) {
					for ( var z = start.z; z <= end.z; z++ ) {
						var data = GetChunk( x, y, z );
...
//						SMLog.Debug( $"{type} : {x},{y},{z}" );
		...
		public SMChunkData GetChunk( int x, int y, int z ) {
