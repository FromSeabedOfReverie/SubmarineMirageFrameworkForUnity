...
namespace SubmarineMirage.Task.Modifyler {
	...
	public abstract class SMBehaviourModifyData : SMLightBase {
		...
		[SMShowLine] public SMBehaviourBody _body	{ get; private set; }
		public SMBehaviourModifyler _owner	{ get; set; }
		...
		public SMBehaviourModifyData( SMBehaviourBody body ) {
			...
			SMLog.Debug( $"{nameof( SMBehaviourModifyData )}() : {this}" );
#endif
		}
		...
		public static void UnLinkBehaviour( SMBehaviourBody body ) {
			var b = body._owner;
#if TestSMTaskModifyler
			SMLog.Debug( $"{nameof( SMBehaviourBody )}.{nameof( UnLinkBehaviour )} : start\n{b}" );
/*
			var p = b._previous;
			var n = b._next;
*/
...
*/
			SMLog.Debug( $"{nameof( SMBehaviourBody )}.{nameof( UnLinkBehaviour )} : end\n{b}" );
#endif
		}
	}
}