...
namespace SubmarineMirage.TestMultiEvent {
	...
	public class TestSMMultiAsyncEvent : SMStandardTest {
		readonly SMMultiAsyncEvent _events = new SMMultiAsyncEvent();
		...
		public IEnumerator TestModifyler() => From( async () => {
			TestSMMultiEventSMUtility.SetModifyler(
				_events,
		...
		public IEnumerator TestChangeWhileRunning() => From( async () => {
			TestSMMultiEventSMUtility.SetChangeWhileRunning(
				_events,
				a => new Func<SMTaskCanceler, UniTask>( async canceler => {
					a();
					SMLog.Debug( "start" );
		...
		public IEnumerator TestManual() => From( async () => {
			...
			_disposables.AddLast(
				TestSMMultiEventSMUtility.SetKey(
					_events,
