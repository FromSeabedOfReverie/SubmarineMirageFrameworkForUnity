...
namespace SubmarineMirage.TestEvent {
	...
	public static class TestSMEventSMUtility {
		public static void SetModifyler<T>( BaseSMEvent events, Func<Action, T> function,
		...
		public static void SetChangeWhileRunning<T>( BaseSMEvent events, Func<Action, T> function,
														Action<string> addEvent = null
		) {
			SMLog.Debug( "・生成" );
			SMLog.Debug( events );
		...
		public static SMDisposable SetKey<T>( BaseSMEvent events, Func<Action, T> function ) {
			var disposables = new SMDisposable();
			var count = 0;
