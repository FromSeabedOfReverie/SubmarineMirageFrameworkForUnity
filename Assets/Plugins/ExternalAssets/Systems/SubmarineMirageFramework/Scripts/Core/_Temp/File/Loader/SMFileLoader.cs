...
namespace SubmarineMirageFramework {
	...
	public class SMFileLoader : BaseSMDataLoader {
		...
		/// ● 要素
		///------------------------------------------------------------------------------------------------
		ISMNetworkManager _networkManager	{ get; set; }
		...
		public override void Setup() {
			_networkManager = SMServiceLocator.Resolve<ISMNetworkManager>();
		...
		public bool IsCanUseCache( SMFileLocation location, bool isWantUseCache ) {
			switch ( location ) {
				...
				case SMFileLocation.Server:
					return isWantUseCache || !_networkManager.IsConnect || !_setting._isRequestUpdateServer;
